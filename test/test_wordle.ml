(* OUnit test suite for Wordle.WordleGame. *)

open OUnit2
open Wordle.WordleGame
...
(* Function to test read_file function. Writes [contents] to a fresh temp file,
   reads it back through read_file, and asserts the parsed lines equal [out].
   Keeping the fixture inline makes each case self-contained and immune to
   editor/whitespace/line-ending drift. *)
let read_file_test out contents =
  let path = Filename.temp_file "wordle_read_file" ".txt" in
  let oc = open_out path in
  output_string oc contents;
  close_out oc;
  let file_name = read_file path in
  Sys.remove path;
...
let read_file_tests =
  [
    "Test read_file | empty file with no text" >:: read_file_test [] "";
    "test_read_file | non-empty file/one line"
    >:: read_file_test [ "Line 1" ] "Line 1\n";
